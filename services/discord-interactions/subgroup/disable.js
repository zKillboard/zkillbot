...
export async function interaction(db, interaction) {
    ...
    const res = await db.subsCollection.updateOne({ channelId: channelId }, { $set: { [`subgroups.${subGroup}.enabled`]: false } });
    if (res.modifiedCount > 0) {
        return `✅ Subscription group **${subGroup}** has been disabled.`;
    } else {
        return `🛑 Subscription group **${subGroup}** was already disabled.`;
    }
